...
const Education = () => {
  return (
    <section id="education" className="py-20 relative bg-dlighter/30">
      <div className="container mx-auto px-6 md:px-12">
        ...
        >
          <h2 className="text-3xl md:text-5xl font-bold mb-4">My <span className="text-pcyan">Education</span></h2>
        ...
        <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-8">
          {educationData.map((edu, index) => (
            <motion.div
              ...
              className="glass p-8 rounded-2xl border border-white/5 hover:border-pblue/30 transition-all group relative overflow-hidden"
            >
              <div className="flex items-start gap-5">
                <div className="w-12 h-12 rounded-xl bg-pblue/10 flex items-center justify-center text-pblue group-hover:bg-pblue group-hover:text-white transition-all">
                  {index === 0 ? <GraduationCap size={24} /> : <Award size={24} />}
                </div>
                <div>
                  <h3 className="text-xl font-bold text-white mb-1">{edu.degree}</h3>
                  <h4 className="text-pblue font-medium mb-3">{edu.institution}</h4>
